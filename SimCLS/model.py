...
from transformers import RobertaModel, BertModel, AutoModel, AutoModelForSeq2SeqLM
...
class ReRanker(nn.Module):
    def __init__(self, encoder, pad_token_id, args):
        super(ReRanker, self).__init__()
        if args.use_checkpoint:
            model = RobertaModel.from_pretrained(encoder)
    ...
    def forward(self, text_id, candidate_id, summary_id=None, require_gold=True):
        ...
        # encoder 计算 此处引入Roberta model作为打分模型
        # Article embedding 
