...
def evaluation(args):
    ...
    for i,v in enumerate(test_set):
        if i<10:
            print(v)
    dataloader = DataLoader(test_set, batch_size=8, shuffle=False, num_workers=args.num_workers, collate_fn=collate_fn)
    # build models
    model_path = args.pretrained if args.pretrained is not None else args.model_type
    scorer = model.ReRanker(model_path, tok.pad_token_id,args)
    if args.cuda:
        scorer = scorer.cuda()
...
def run(rank, args):
    ...
    if is_mp:
      ...
      train_set, num_replicas=world_size, rank=rank, shuffle=True)
        dataloader = DataLoader(train_set, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers, collate_fn=collate_fn, sampler=train_sampler)
        val_sampler = torch.utils.data.distributed.DistributedSampler(
      val_set, num_replicas=world_size, rank=rank)
        val_dataloader = DataLoader(val_set, batch_size=8, shuffle=False, num_workers=args.num_workers, collate_fn=collate_fn_val, sampler=val_sampler)
    else:
        dataloader = DataLoader(train_set, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, collate_fn=collate_fn)
        val_dataloader = DataLoader(val_set, batch_size=8, shuffle=False, num_workers=args.num_workers, collate_fn=collate_fn_val)
    # build models
    model_path = args.pretrained if args.pretrained is not None else args.model_type
    scorer = model.ReRanker(model_path, tok.pad_token_id,args)
    ...
    with torch.autograd.profiler.emit_nvtx():
        torch.cuda.nvtx.range_push("training simcls baseline")
        for epoch in range(args.epoch):
            ...
            for (i, batch) in enumerate(dataloader):
                if args.half_precision:
                    with torch.autocast(device_type='cuda', dtype=torch.float16):
                        torch.cuda.nvtx.range_pop()#data load 1
                        if args.cuda:
